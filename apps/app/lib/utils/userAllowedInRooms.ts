...
function checkDefaultAccess({ room, accessAllowed }: UserAllowedInRoomProps) {
  ...
  if (accessAllowed === 'write' && defaultAccess.includes('room:write')) {
    return true;
  }
...
function checkUserAccess({
  ...
  if (accessAllowed === 'write' && userAccesses.includes('room:write')) {
    return true;
  }
  ...
  if (
    accessAllowed === 'read' &&
    (userAccesses.includes('room:write') || userAccesses.includes('room:read'))
  ) {
    return true;
  }
